    ...
    $.ajax({
        ...
        success: data => {
            ...
            $.ajax({
                ...
                success: output => {
                    //console.log(output);
                    //console.log(output[0].stdData[0].contactNumber);
                    var msgData = {
...
$(document).on("click", ".generateBill", function() {
    ...
    var billData = $('#projectsByStudio').find('input[type="checkbox"]:checked').parent()
    ...
    for (let i = 0; i < billData.length; i++) {
        ...
        `
        console.log(billData[i].parentElement.innerHTML)
    }
})